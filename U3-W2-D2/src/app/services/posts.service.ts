...
export class PostsService {
  ...
  getAllPosts() {
    return this.postJsonArray;
  }

  getActivePosts() {
    return this.postJsonArray.filter((post) => post.active);
  }
  ...
  getTopPosts() {
    const index = Math.floor(Math.random() * this.postJsonArray.length);
    return this.postJsonArray[index];
  }

  getRelatedPosts(num: number) {
    const presenti: number[] = [];
    const related: any[] = [];
    for (let i = 0; i < num; i++) {
      const indice = Math.floor(Math.random() * this.postJsonArray.length);
      if (presenti.includes(indice)) this.getRelatedPosts(num - i);
      presenti.push(indice);
      related.push(this.postJsonArray[indice]);
    }
    return related;
  }

  getPostById(id: number) {
    return this.postJsonArray.find((post) => post.id === id);
  }

  getAllTags() {
    const tags: string[] = [];
    this.postJsonArray.forEach((post) => {
